...
function Trainer(): JSX.Element {
  ...
  const handleMove = useCallback(
    (uci: string): void => {
      ...
      // Async eval — update log entry and eval bar when result arrives
    ...
    [currentBlunder, localFen, firstMove, botMode],
  );
  ...
  async function handleShowBlunderSequence(): Promise<void> {
    ...
    const playStep = (moves: string[], fen: string, index: number): void => {
      ...
      setTimeout(() => playStep(moves, newFen, index + 1), 600);
    };

    // Small initial delay so the reset board renders before the sequence starts.
    setTimeout(() => playStep(result.moves, currentBlunder.fen_before, 0), 200);
  }
    ...
    <div className="trainer">
      ...
      <div className="trainer__board-row" ref={boardRowRef}>
        <div className="trainer__eval" style={sideHeightStyle}>
          <EvalBar cpScore={currentEval} orientation={orientation} />
        </div>

        <div className="trainer__board" style={boardDimStyle}>
          <Board
            fen={localFen || currentBlunder.fen_before}
        ...
        <div className="trainer__panel" style={sideHeightStyle}>
          {/* Move log */}
