        ...
        if threshold != CANONICAL_THRESHOLD:
            if time_class == "all":
                game_rows = conn.execute(
                    "SELECT game_url, player_color FROM user_analysed_games WHERE username_lower = ?",
                    (username_lower,),
                ).fetchall()
            else:
                game_rows = conn.execute(
                    "SELECT game_url, player_color FROM user_analysed_games WHERE username_lower = ? AND time_class = ?",
                    (username_lower, time_class),
                ).fetchall()
        else:
            game_rows = []
